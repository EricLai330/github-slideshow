...
export class Tab4Page {
 ...
 showLegend = true;
  ...
  getData() {
    ...
    var baseDiv:number;
    if(this.global.logCount<=16) 
      baseDiv=0;
    else
    {
      baseDiv = (this.global.logCount - this.global.logCount % 16)/16;
    }
    for(var i=0;i<this.global.logCount;i++)
    {
      if ( baseDiv==0 )
        this.chartLabels.push(this.csvData[i][0].slice(5,16));
      else
        {
          if((i%baseDiv)==0) 
          {
            this.chartLabels.push(this.csvData[i][0].slice(5,16));
            console.log('showing baseDiv=',i);
          }
        }
...
//    var strTime = days + '/' + month + '/' + year + '/ '+hours + ':' + minutes;
   ...
   //       this.T4LoadEvent(); //reading 2048
    ...
    getEvent(){
      ...
      for (var i=0;i<this.global.eventCount;i++)
      {
