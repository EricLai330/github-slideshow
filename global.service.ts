...
//      console.log('buffer byteLength',num);
 ...
 //buffer.byteLength,this.dslen,this.dslenTotal);
      if( this.dslen==261 )
      {
        ...
        for (var i=0;i<16;i++)
        {
          ttime=this.dsOneTemp[i*16+0]*256+this.dsOneTemp[i*16+1];;
          ...
          {
            if(ttime<0xFFEF) //not empty, re-read
            {
              console.log('legit fail ttime=',ttime);
              ...
              this.BleWrite(); //re-read same log
              return;
            }
          } //not legit
        } //for
...
unPackLog()
{
  ...
  for (j = 0; j < 16; j++)
  {
    for (i=0; i < 16; i++)
    {
      ...
      {
        console.log('attention log: over the time limit,time=,i=,j=,count=',time,i,j,this.logCount);
      }
    }// for i
  } // for j
  ...
  parseEvent()
  {
    ...
    for (j = 0; j < 8; j++)
    {
      for (i=0; i < 16; i++)
      {
        ...
        {
          console.log('attention event: over the time limit,time=,i=,j=,count=',time,i,j,this.eventCount);
        }        
      }// for i
    } // for j
    console.log ('total event Count=',this.eventCount);
    j=this.nowTimeCount;
...
//sorting time
    ...
    let tArray:any;
    for(a=0;a<this.eventCount;a++)
    { 
      for(b=(a+1);b<this.eventCount;b++)
      {
        numa = this.timeTwo[a];
        numb = this.timeTwo[b];
        if(numa < numb)
